...
def is_prime(N):
    ...
    for i in range(2,int(N**0.5)+1): #1은 소수가 아니므로 2부터 N의 제곱근까지를 범위로 함.
    #제곱근을 대칭으로 쌍을 이루어 N의 약수가 되므로 한 쪽만 계산해도 배수들이 제외됨
        if array[i] == True: #제거되지 않은 숫자인가를 판별
            j = 2 #j는 제거되지 않은 가장 작은 수를 의미
            while i*j <= N:
                array[i*j] = False #j의 배수라면 소수가 아니므로 제거
                j+=1 #i*j가 N을 초과할 때까지 j를 늘려가며 곱하고 그 값은 배수이므로 제거
